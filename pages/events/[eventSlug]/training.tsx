...
import SpeedZoneOverspeedListComponent from "components/events/training/speedZoneOverspeed";
...
const Replay: NextPage<EventProps> = (props) => {
  ...
  const [overSpeedMargin, setOverSpeedMargin] = useState<number>(0);
  const [overSpeedDuration, setOverSpeedDuration] = useState<number>(0);

  const participantFilter = useRef<HTMLSelectElement>(null);
  const startDate = useRef<HTMLInputElement>(null);
  const endDate = useRef<HTMLInputElement>(null);
  const overSpeedMarginRef = useRef<HTMLInputElement>(null);
  const overSpeedDurationRef = useRef<HTMLInputElement>(null);
  ...
  const [speedZoneOverCount, setSpeedZoneOverCount] = React.useState<number>(0);
  ...
  // const onOverSpeedDurationTypeChange = () => {};

  // const onOverSpeedDurationChange = () => {
  //   if (overSpeedDurationRef.current !== null) {
  //     setOverSpeedDuration(overSpeedDurationRef.current.valueAsNumber);
  //   }
  // };

  const onOverSpeedMarginChange = () => {
    if (overSpeedMarginRef.current !== null) {
      setOverSpeedMargin(overSpeedMarginRef.current.valueAsNumber);
    }
  };
  ...
  const onTotalSpeedZonesCount = (total: number) => {
    setSpeedZoneOverCount(total);
  };
    ...
    ppTrackerClient.getIncidencesData(
      ...
      (data: apiIncidence[], error?: unknown) => {
        ...
        if (error) {
          ...
          setSpeedZoneOverCount(speedZoneOver);
            ...
            <Col>
              <Form>
                ...
                <Row>
                  <Col>
                    <Form.Group className="mb-3" controlId="startDateInput">
                      <Form.Label>Overspeed Margin:</Form.Label>
                      <Form.Control
                        className="sm fw-bold"
                        type="number"
                        min={0}
                        max={100}
                        defaultValue={overSpeedMargin}
                        ref={overSpeedMarginRef}
                        onChange={onOverSpeedMarginChange}
                      />
                    </Form.Group>
                  </Col>
                  <Col>
                    <Form.Group className="mb-3" controlId="startDateInput">
                      ...
                      <Form.Select ref={durationTypeRef}>
                  ...
                  <Col>
                    <Form.Group className="mb-3" controlId="startDateInput">
                      ...
                      <Form.Control
                        ...
                        min={0}
                        max={9999}
                        defaultValue={overSpeedDuration}
                        ref={overSpeedDurationRef}
                        onChange={onOverSpeedDurationChange}
                      />
                    </Form.Group>
                  </Col>
                </Row>
              ...
              >
                ...
                >
                  <OverspeedListComponent
                    ...
                    rally={rally}
                    data={incidences}
                    overspeedMargin={overSpeedMargin}
                ...
                >
                  <SpeedZoneOverspeedListComponent
                    event={activeEvent}
                    rally={rally}
                    data={incidences}
                    overspeedMargin={overSpeedMargin}
                    onTotalOverSpeeds={onTotalSpeedZonesCount}
                  />
                </Tab>
              </Tabs>
            </Col>
